...
- **Bounded-stream sentinels, not `done`/`is_last` flags**: `messages.control.EndOfClipMessage` and `messages.llm.EndOfGenerationMessage` are payload-free control messages sent once, after the last real message in a bounded run, matched by `isinstance` — not a boolean field on every payload message. A stage that must accumulate across such a run keeps the accumulation as instance state and flushes on the sentinel; a stage that doesn't care just never matches it.
