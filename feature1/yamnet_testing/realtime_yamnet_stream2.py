...
class RealtimeYAMNet:
    ...
    def __init__(self, config):
        ...
        # Find scream indices if detecting screams
        if config.detect_screams:
            self.scream_indices = self.find_scream_indices()
            print(f"Scream detection enabled ({len(self.scream_indices)} classes)")
    ...
    def load_model(self):
        ...
        # Determine how to reshape audio based on expected input
        self.input_ndim = len(expected_shape)
        if self.input_ndim == 4:
            # Model expects [batch, time, 1, 1] format
            print("Model expects 4D waveform input [batch, time, 1, 1]")
        elif self.input_ndim == 2:
            # Model expects [batch, time] format
            print("Model expects 2D waveform input [batch, time]")
        elif self.input_ndim == 1:
            # Model expects raw waveform
            print("Model expects 1D waveform input")
    ...
    def run_inference(self, audio_chunk):
        ...
        if self.input_ndim == 4:
            # YAMNet format: [batch, time, 1, 1]
            audio_input = audio_chunk.reshape(1, -1, 1, 1)
        elif self.input_ndim == 2:
            # Batch format: [batch, time]
    ...
    def postprocess_output(self, output_data, top_k=5):
        ...
        # Check for scream if enabled
        is_scream = False
        max_scream_score = 0
        if self.config.detect_screams:
            scream_scores = scores[self.scream_indices]
            max_scream_score = np.max(scream_scores)
            is_scream = max_scream_score >= self.config.scream_threshold
        
        t_end = time.perf_counter()
        self.perf_monitor.record('postprocessing', (t_end - t_start) * 1000)
        
        return predictions, is_scream, max_scream_score
        ...
        while self.running:
            try:
                ...
                if len(self.audio_buffer) >= self.config.window_size:
                    ...
                    # Postprocess
                    predictions, is_scream, scream_score = self.postprocess_output(output)
    ...
    def display_thread(self):
        ...
        while self.running:
            try:
                ...
                if self.config.detect_screams:
                    if result['is_scream']:
                        print(f"🔴 SCREAM DETECTED | Score: {result['scream_score']:.3f}")
                    else:
                        print(f"⚪ No scream | Max: {result['scream_score']:.3f}")
