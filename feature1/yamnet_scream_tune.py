...
class AudioConfig:
    ...
    def __init__(self):
        ...
        self.debug = '--debug' in sys.argv          # print full group report every chunk

        # Threat score threshold to fire an alert [0, 1]
        # Tune this after observing threat scores on real audio.
        self.threat_threshold = 0.30
...
class RealtimeYAMNet:
    ...
    def __init__(self, config):
        ...
        # Class grouper — maps all 521 YAMNet classes to semantic groups
        self.grouper = ClassGrouper(self.labels)

        if config.detect_screams:
            print(f"Security detection enabled")
            print(f"  Threat threshold : {config.threat_threshold}")
            print(f"  Confirmation     : {config.confirmation_frames} frames")
    ...
    def postprocess_output(self, output_data, top_k=5):
        """
        Post-process YAMNet output using semantic class grouping.

        1. Aggregate all 521 YAMNet scores into semantic groups.
        2. score_threat() applies binary group activation and shouted-speech
           bonus to produce a clean threat percentage.
        3. Temporal confirmation filters single-frame spikes.
        """
        t_start = time.perf_counter()
        ...
        # Always aggregate — group scores needed for both debug and detection
        group_scores  = self.grouper.aggregate(scores)
        threat_result = score_threat(group_scores)
        threat_score  = threat_result["threat_score"]

        # Top-k predictions for display
        top_indices = np.argsort(scores)[-top_k:][::-1]
        predictions = [(self.labels[idx], float(scores[idx])) for idx in top_indices]
        ...
        if self.config.detect_screams:
            ...
            is_threat = self.detection_streak >= self.config.confirmation_frames

        t_end = time.perf_counter()
        self.perf_monitor.record('postprocessing', (t_end - t_start) * 1000)

        return predictions, is_threat, threat_score, threat_result, group_scores
        ...
        while self.running:
            try:
                ...
                if len(self.audio_buffer) >= self.config.window_size:
                    ...
                    processed_window = self.preprocess_audio(window_to_process)

                    # Run YAMNet inference
                    output = self.run_inference(processed_window)

                    # Postprocess — grouper + threat scorer
                    predictions, is_threat, threat_score, threat_result, group_scores = \
    ...
    def display_thread(self):
        ...
        while self.running:
            try:
                ...
                tr = result['threat_result']
                gs = result['group_scores']

                # Full group breakdown every chunk — useful for calibrating weights
                if self.config.debug:
                    print_group_report(gs, tr)

                # Scream detection indicator
                if self.config.detect_screams:

                    threat_bar = "█" * min(30, int(result['threat_score'] * 30))
                    print(f"Threat     : {result['threat_score']:.3f}  [{threat_bar:<30s}]  "
                          f"(threshold={self.config.threat_threshold})")

                    # Show which groups are contributing
                    active_groups = {
                        g: s for g, s in gs.items()
                        if s >= 0.08 and g not in ("ignore", "unmatched")
                    }
                    if active_groups:
                        print("Active groups:")
                        for g, s in sorted(active_groups.items(), key=lambda x: -x[1]):
                            print(f"   {g:30s} {s:.3f}")

                    if tr.get("weapon_triggered"):
                        print(f"⚠️  WEAPON CLASS TRIGGERED")

                    print(f"Streak     : {self.detection_streak}/{self.config.confirmation_frames} frames needed")

                    if result['is_threat']:
                        print(f"🔴 SECURITY ALERT | Threat score: {result['threat_score']:.3f}")
                    elif result['threat_score'] >= self.config.threat_threshold:
                        print(f"🟡 BUILDING ({result['threat_score']:.3f} ≥ {self.config.threat_threshold}, "
                ...
                for i, (label, score) in enumerate(result['predictions'], 1):
                    ...
                    # Highlight vocal distress classes
                    is_distress = self.config.detect_screams and any(
                        self.grouper._index_to_group.get(idx, "") == "vocal_distress"
                        for idx, lbl in self.labels.items() if lbl == label
                    )
                    prefix = "🎯 " if is_distress else "   "
    ...
    def stop(self):
        ...
        if self.config.detect_screams:
            print(f"Total threats detected: {self.scream_count}")
            print(f"Threat rate: {100*self.scream_count/max(1,self.total_inferences):.1f}%")
