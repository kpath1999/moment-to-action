...
def get_best_frames(frame_paths: Iterable[str], llm_query: str, pretrained_path: str, top_k: int = 2) -> List[str]:
	"""
	Return top-k frame paths most similar to the LLM-provided query using MobileCLIP.
	"""
	...
	topk = scores.squeeze().topk(min(top_k, len(frame_paths)))
	return [frame_paths[i] for i in topk.indices]
