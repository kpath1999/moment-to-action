...
class TestRTMDetModelLoadUnload:
    ...
    def test_load_onnx_calls_load_model(
        self, onnx_model: RTMDetModel, mock_platform: MagicMock
    ) -> None:
    ...
    def test_load_dlc_calls_load_model_dlc(
        self, dlc_model: RTMDetModel, mock_platform: MagicMock
    ) -> None:
    ...
    def test_load_dlc_none_data_type_raises(self, mock_platform: MagicMock) -> None:
        ...
        model = RTMDetModel("qcs6490", Path("/fake/qcs6490"), ModelType.DLC, backends=_DLC_BACKENDS)
    ...
    def test_load_twice_raises(self, onnx_model: RTMDetModel, mock_platform: MagicMock) -> None:
...
class TestRTMDetModelRun:
    ...
    def test_run_onnx_calls_backend_run(
        self, onnx_model: RTMDetModel, mock_platform: MagicMock, sample_image_array: np.ndarray
    ) -> None:
    ...
    def test_run_dlc_returns_tensor_list(
        self, dlc_model: RTMDetModel, mock_platform: MagicMock, sample_image_array: np.ndarray
    ) -> None:
        """run() with DLC format returns [boxes, scores, class_idx] from handle.run."""
