...
def onnx_model() -> RFDETRModel:
    ...
    return RFDETRModel("default", Path("/fake/model.onnx"), ModelFormat.ONNX)


@pytest.fixture
def dlc_model() -> RFDETRModel:
    """Return an unloaded RFDETRModel in DLC format (qcs6490 → NHWC auto-detected)."""
    return RFDETRModel("qcs6490", Path("/fake/qcs6490"), ModelFormat.DLC)


@pytest.fixture
def dlc_model_nchw() -> RFDETRModel:
    """Return an unloaded RFDETRModel in DLC format with NCHW layout (non-qcs6490 variant)."""
    return RFDETRModel("other", Path("/fake/other"), ModelFormat.DLC)
...
def _make_outputs(
    ...
    """Build synthetic 3-tensor RF-DETR output."""
...
@pytest.mark.unit
class TestRFDETRModelPrepare:
    """Tests for RFDETRModel.prepare()."""

    def test_nchw_output_shape(
        self, onnx_model: RFDETRModel, sample_image_array: np.ndarray
    ) -> None:
        """prepare() returns NCHW (1, 3, 560, 560) for ONNX."""
        result = onnx_model.prepare(sample_image_array)
        assert result.shape == (1, 3, 560, 560)

    def test_dlc_nchw_output_shape(
        self, dlc_model_nchw: RFDETRModel, sample_image_array: np.ndarray
    ) -> None:
        """prepare() returns NCHW (1, 3, 560, 560) for non-qcs6490 DLC."""
        result = dlc_model_nchw.prepare(sample_image_array)
        assert result.shape == (1, 3, 560, 560)

    def test_dlc_nhwc_output_shape(
        self, dlc_model: RFDETRModel, sample_image_array: np.ndarray
    ) -> None:
        """prepare() returns NHWC (1, 560, 560, 3) for qcs6490 AI Hub DLC."""
        assert dlc_model.input_layout == "NHWC"
        result = dlc_model.prepare(sample_image_array)
        assert result.shape == (1, 560, 560, 3)
...
@pytest.mark.unit
class TestRFDETRModelProperties:
    """Tests for RFDETRModel properties."""

    def test_confidence_threshold_default(self, onnx_model: RFDETRModel) -> None:
    ...
    def test_confidence_threshold_custom(self) -> None:
        ...
        model = RFDETRModel("default", Path("/f"), ModelFormat.ONNX, confidence_threshold=0.3)
        assert model.confidence_threshold == 0.3

    def test_input_layout_onnx_is_nchw(self, onnx_model: RFDETRModel) -> None:
        """ONNX variant uses NCHW layout."""
        assert onnx_model.input_layout == "NCHW"

    def test_input_layout_qcs6490_is_nhwc(self, dlc_model: RFDETRModel) -> None:
        """qcs6490 DLC variant uses NHWC layout."""
        assert dlc_model.input_layout == "NHWC"

    def test_input_layout_other_dlc_is_nchw(self, dlc_model_nchw: RFDETRModel) -> None:
        """Non-qcs6490 DLC variant uses NCHW layout."""
        assert dlc_model_nchw.input_layout == "NCHW"


@pytest.mark.unit
class TestRFDETRModelLoadUnload:
    ...
    def test_load_dlc_calls_load_model_dlc(
        ...
        with MagicMock() as mock_resolve:
            import moment_to_action.models.image.detection.rf_detr._model as m
...
@pytest.mark.unit
class TestRFDETRModelRun:
    """Tests for RFDETRModel.run()."""
    ...
    def test_run_dlc_returns_tensor_list(
        ...
        np.testing.assert_array_equal(result[1], logits)
        np.testing.assert_array_equal(result[2], classes)


@pytest.mark.unit
class TestRFDETRModelPostProc:
    """Tests for RFDETRModel.post_proc() and _decode()."""

    def test_returns_empty_on_no_detections(self, onnx_model: RFDETRModel) -> None:
        """Empty outputs return no detections."""
        outputs = _make_outputs([], [], [])
        result = onnx_model._decode(outputs, original_size=None)
        assert result == []

    def test_filters_low_confidence(self, onnx_model: RFDETRModel) -> None:
        """Detections below confidence_threshold are discarded."""
        outputs = _make_outputs([[0, 0, 100, 100]], [0.1], [0])
        result = onnx_model._decode(outputs, original_size=None)
        assert result == []

    def test_high_confidence_detection_kept(self, onnx_model: RFDETRModel) -> None:
    ...
    def test_returns_fewer_than_three_outputs_empty(self, onnx_model: RFDETRModel) -> None:
        """Fewer than 3 output tensors returns empty list."""
        result = onnx_model._decode([np.zeros((1, 5, 4)), np.zeros((1, 5))], original_size=None)
        assert result == []

    def test_scales_to_original_size(self, onnx_model: RFDETRModel) -> None:
        """Boxes are scaled to the original frame size."""
        outputs = _make_outputs([[0, 0, 560, 560]], [0.9], [0])
        result = onnx_model._decode(outputs, original_size=(1120, 1120))
        assert len(result) == 1
        b = result[0].bbox
        assert b.x2 == pytest.approx(1120.0)
        assert b.y2 == pytest.approx(1120.0)

    def test_coco_label_mapping(self, onnx_model: RFDETRModel) -> None:
        """Class ID maps to the correct COCO label."""
        outputs = _make_outputs([[0, 0, 100, 100]], [0.9], [2])
        result = onnx_model._decode(outputs, original_size=None)
        assert result[0].label == "car"

    def test_unknown_class_id_uses_string(self, onnx_model: RFDETRModel) -> None:
    ...
    def test_nms_suppresses_overlapping(self, onnx_model: RFDETRModel) -> None:
    ...
    def test_detection_has_correct_fields(self, onnx_model: RFDETRModel) -> None:
    ...
    def test_decode_uses_provided_size(self, onnx_model: RFDETRModel) -> None:
        """decode() uses the caller-supplied original_size."""
        outputs = _make_outputs([[0, 0, 560, 560]], [0.9], [0])
        result = onnx_model.decode(outputs, original_size=(280, 280))
        assert result[0].bbox.x2 == pytest.approx(280.0)
        assert result[0].bbox.y2 == pytest.approx(280.0)


@pytest.mark.unit
class TestRFDETRModelNMS:
    """Tests for RFDETRModel._nms()."""

    def test_single_box_kept(self) -> None:
        """Single box is always kept."""
        boxes = np.array([[0, 0, 100, 100]], dtype=np.float32)
        scores = np.array([0.9], dtype=np.float32)
        keep = RFDETRModel._nms(boxes, scores, iou_threshold=0.5)
        assert keep == [0]
