...
class RTMDetModel(ImageDetectionModel):
    ...
    RTMDet outputs three homogeneous-range tensors after AI Hub export:
    ...
    def prepare(self, frame: np.ndarray) -> np.ndarray:
        ...
        resized = cv2.resize(frame, (_RTMDET_INPUT_SIZE, _RTMDET_INPUT_SIZE))
    ...
    def _decode(
        ...
        """Parse RTMDet 3-output format, filter, NMS, and scale.
        ...
        if original_size is not None:
            ...
            sx = orig_w / float(_RTMDET_INPUT_SIZE)
            sy = orig_h / float(_RTMDET_INPUT_SIZE)
        else:
            sx, sy = 1.0, 1.0
            orig_w, orig_h = _RTMDET_INPUT_SIZE, _RTMDET_INPUT_SIZE
