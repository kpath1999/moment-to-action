...
class MobileCLIPStage(Stage):
    ...
    def __init__(
        ...
        self._handles = [
                backend.load_model(model_path) for _ in range(num_workers)
        ]

        # Pool of free interpreter handles — workers grab one, use it, return it
        self._handle_pool: queue.Queue = queue.Queue()
        for h in self._handles:
            self._handle_pool.put(h)
        ...
        self._text_embeddings = self._precompute_text_embeddings(self._handles[0])
        logger.info("MobileCLIPStage: loaded %s with %d prompts", model_path, len(text_prompts))

        # Persistent thread pool — no spin-up cost per batch
        self._executor = ThreadPoolExecutor(max_workers=num_workers)

    def _precompute_text_embeddings(self, handle: object) -> np.ndarray:
    ...
    #        results = []
    #        for frame_msg in msg:
    #            result = self._process_single(frame_msg)
    #            results.append(result)
    #        return results                          # or return aggregated result here later
    #    return self._process_single(msg)

    def _process(self, msg, _metrics):
        if isinstance(msg, list):
            # Submit all frames concurrently, preserve original order
            futures = {
                self._executor.submit(self._process_single, frame_msg): i
                for i, frame_msg in enumerate(msg)
            }
            results = [None] * len(msg)
    ...
    def _process_single(self, msg: FrameTensorMessage) -> ClassificationMessage:
        ...
        try:
            ...
            self._handle_pool.put(handle)  # always return, even on error
        ...
        print(f"  frame ts={msg.timestamp:>4}  →  '{label}'  ({confidence:.3f})")
